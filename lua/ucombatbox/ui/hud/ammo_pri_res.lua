local PLUG = {}
PLUG.Name = "Ammo Reserve"

local am_mat = Material("icon16/basket.png")
...
local function ammo_primary_max(ply)
	...
	local wep = ply:GetActiveWeapon()
	if not IsValid(wep) then return -1 end
	return (wep.GetMaxClip1 != nil and wep:GetMaxClip1() > 0) and (wep:GetMaxClip1()*4) or 6
end
...
function PLUG:Draw(x,y,w,h,mx,my)
	...
	local ammo_max = ammo_primary_max(LocalPlayer())
	self:DrawBarInv(up,x,y,w,h,ammo,ammo_max,Angle(89,93,0),am_mat,Color(50,185,16),Color(25,25,25,150),1.3)
end
